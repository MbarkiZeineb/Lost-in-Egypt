...
{
anubis->position.x=820;
anubis->position.y=350;
anubis->fond1 = IMG_Load("e1.png");
}

void afficheranubis(anubis anubis,SDL_Surface *ecran,SDL_Rect camera)
{ if((camera.x>1950&&camera.x<2000) || (camera.x>2450&&camera.x<2500))
  
  {SDL_BlitSurface(anubis.fond1,NULL, ecran,&anubis.position);}	
}
...
int r ;
r = rand();
if((ennemi.position.x<=0||ennemi.position.x>=1000)&&(camera.x>0&&camera.x<8000))
{
if(r%6==0)
{
